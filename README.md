...
|---|---|---|---|---|---|---|
| [Betfair Exchange](https://www.betfair.com/exchange) | ✓ | ✓ | partial | odds ticks+book | — | No compliant free path to volume: the free Delayed App Key explicitly omits totalMatched, and the GBP 499 Live key forbids read-only data collection without commensurate betting. History is a separate paid product. Every anonymous route returns 403 behind bot protection. |
...
| [PredictIt](https://www.predictit.org) | ✓ | — | none | snapshot quotes | — | The public API is a price snapshot with no volume. Daily share volume does exist per market via GetMarketChartData, but it needs US egress (other regions get a Cloudflare country block), rate-limits to ~15 req/min across ~197 markets, reports shares rather than USD, and its terms restrict use to non-commercial with attribution. |
...
| [Smarkets](https://smarkets.com) | ✓ | ✓ | none | live odds+book | — | A documented, unauthenticated /volumes/ endpoint exists, but Smarkets API terms prohibit data extraction without commensurate trading, redistribution, and benchmarking of liquidity — so this directory does not call it. Volume is also wiped on settlement, leaving no 30d flow to recover. |
